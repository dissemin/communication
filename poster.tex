...
\block{Fostering open access}{
    ...
    Universities set up open access policies to ensure that their
    researchers upload their papers to repositories.

    But bibliographic metadata is scattered in many different places,
    under diverse formats, making the implementation of these policies
    cumbersome.

    We provide a clear view on the research output of researchers by harvesting
...
\block{Data sources}{
    ...
    \begin{itemize}[leftmargin=0pt]
       \item Bibliographic data issued by the publishers, through
            the CrossRef.org API;
       \item Full text availability in repositories, through
            the preprint search engines BASE (Bielefeld Academic
            Search Engine) and CORE (COnnecting REpositories), plus
            our own OAI-PMH harvester to ensure we have the latest
